 ...
 * Optimisation Responsive Mobile & Tactile
 */
...
const AppModule = {
  ...
  bindNavigation() {
    // Navigation Sidebar Desktop & Tiroir Mobile
    ...
    // Navigation Barre Inférieure Mobile (Bottom Nav)
    document.querySelectorAll(".mobile-bottom-nav-item[data-view]").forEach(item => {
      item.addEventListener("click", (e) => {
        e.preventDefault();
        const targetView = item.getAttribute("data-view");
        this.switchView(targetView);
  ...
  switchView(viewName) {
    ...
    // Mise à jour de la Sidebar (Desktop)
    document.querySelectorAll(".nav-item[data-view]").forEach(item => {
    ...
    // Mise à jour de la Barre Inférieure Mobile (Bottom Nav)
    document.querySelectorAll(".mobile-bottom-nav-item[data-view]").forEach(item => {
      if (item.getAttribute("data-view") === viewName) {
        item.classList.add("active");
      } else {
    ...
    // Fermer le tiroir mobile et son backdrop si ouvert
    const sidebar = document.getElementById("main-sidebar");
    const backdrop = document.getElementById("sidebar-backdrop");
    if (sidebar) sidebar.classList.remove("mobile-open");
    if (backdrop) backdrop.classList.remove("active");

    // Affichage de la vue cible
    ...
    if (target) {
      ...
      if (titleEl) {
        const titles = {
          ...
          availability: "Planning & Disponibilité",
          fleet: "Flotte Véhicules",
          bookings: "Réservations & Contrats",
          customers: "Fichier Clients",
          finance: "Caisse & Règlements",
          settings: "Agence & Sauvegarde"
        };
  ...
  renderHeaderInfo() {
    ...
    if (phoneEl) {
      phoneEl.href = `tel:${(db.agency.phone1 || '+212660990242').replace(/\s+/g, '')}`;
    }
  },
  ...
  renderDashboardAlerts(db) {
    ...
    if (activeRentals.length > 0) {
      alerts.push({
        type: "info",
        icon: "🚗",
        text: `<strong>${activeRentals.length} véhicule(s) actuellement en circulation</strong>.`
      });
    }

    if (alerts.length === 0) {
      alertsContainer.innerHTML = `
        <div style="padding: 10px 14px; border-radius: var(--radius-sm); background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); color: #10B981; font-size: 0.82rem; font-weight: 600;">
          ✓ Flotte à jour. Aucun incident signalé.
        </div>
      `;
    } else {
      alertsContainer.innerHTML = alerts.map(a => `
        <div style="display: flex; align-items: center; gap: 10px; padding: 10px 14px; margin-bottom: 8px; border-radius: var(--radius-sm); background: rgba(245, 158, 11, 0.1); border: 1px solid rgba(245, 158, 11, 0.3); color: #FFF; font-size: 0.82rem;">
  ...
  renderRecentBookingsTable(db) {
    ...
    if (recent.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" style="text-align: center; color: var(--text-dim); padding: 16px;">Aucune réservation enregistrée.</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = recent.map(b => {
      ...
      const startFmt = new Date(b.pickupDate).toLocaleDateString("fr-FR", { day: '2-digit', month: '2-digit' });
      const endFmt = new Date(b.dropoffDate).toLocaleDateString("fr-FR", { day: '2-digit', month: '2-digit' });
      ...
      return `
        <tr>
          ...
          <td>${startFmt} → ${endFmt} (${b.days}j)</td>
          <td><strong>${b.totalAmount} DH</strong></td>
          <td>${statusBadge}</td>
          <td style="text-align: right;">
            <button class="btn btn-sm btn-secondary" onclick="ContractModule.openContract('${b.id}')">📄 Contrat</button>
          </td>
        </tr>
      `;
    }).join("");
  ...
  renderAvailabilityView() {
    ...
    headerRow.innerHTML = `<th class="timeline-car-col">Véhicule</th>` +
      days.map(d => {
        ...
        return `
          <th class="${isToday ? 'today' : ''}">
            <div>${dayName}</div>
            <div style="font-size: 0.95rem; font-weight: 800;">${dayNum}</div>
          </th>
        `;
      }).join("");
    ...
    container.innerHTML = db.fleet.map(car => {
      ...
      const dayCells = days.map(d => {
        ...
        if (match) {
          ...
          return `
            <td class="timeline-slot" onclick="ContractModule.openContract('${match.id}')" title="Réservation ${match.contractNumber}">
          ...
          return `
            <td class="timeline-slot" onclick="BookingModule.openNewBookingModal('${car.id}', '${d.toISOString().slice(0, 10)}T10:00')" style="cursor: pointer;">
              <span style="opacity: 0.2; font-size: 0.75rem;">+</span>
            </td>
          `;
        }
      }).join("");
      ...
      return `
        <tr>
          <td class="timeline-car-col">
            <div style="font-size: 0.8rem; font-weight: 800; color: #FFF; line-height: 1.2;">${car.brand} ${car.model}</div>
            <div style="font-size: 0.68rem; color: var(--gold-400); font-family: monospace;">${car.matricule} • ${car.priceDay} DH</div>
          </td>
          ${dayCells}
        </tr>
      `;
          ...
          <div class="car-body">
            ...
            <div class="car-pricing-box">
              <div>
                ...
                <span class="price-period">/j</span>
              </div>
  ...
  toggleCarStatus(carId) {
    ...
    this.showToast(`${car.brand} ${car.model} : ${car.status.toUpperCase()}`, "info");
    this.refreshAll();
  },
  ...
  deleteCar(carId) {
    ...
    StorageManager.save(db);
    this.showToast("Véhicule supprimé", "warning");
    this.refreshAll();
  },
  ...
  renderBookingsView() {
    ...
    if (db.bookings.length === 0) {
      tbody.innerHTML = `<tr><td colspan="8" style="text-align: center; padding: 20px; color: var(--text-dim);">Aucune réservation trouvée.</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = db.bookings.map(b => {
      ...
      let actionButtons = `
        <button class="btn btn-sm btn-secondary" onclick="ContractModule.openContract('${b.id}')" title="Voir Contrat">📄</button>
      `;
    ...
    tbody.innerHTML = db.customers.map(c => {
      ...
      return `
        <tr>
          ...
          <td>${c.cin || c.passport || '-'}</td>
          <td>${c.permisNumber || '-'}</td>
          <td>
            <a href="tel:${c.phone}" style="color: #FFF; text-decoration: none;">${c.phone}</a><br>
            <a href="https://wa.me/${waNumber}" target="_blank" style="color: #10B981; font-size: 0.72rem; text-decoration: none;">💬 WhatsApp</a>
          </td>
          <td>${c.address || 'Marrakech'}</td>
          <td><span class="badge-disponible" style="padding: 2px 6px; border-radius: 8px; font-size: 0.7rem;">${c.totalRentals || 1} loc.</span></td>
          <td style="text-align: right;">
            <button class="btn btn-sm btn-primary" onclick="BookingModule.openNewBookingModal(null, null, null)">Louer</button>
          </td>
        </tr>
      `;
    }).join("");
  ...
  renderFinanceView() {
    ...
    tbody.innerHTML = db.payments.map(p => `
      <tr>
        ...
        <td>${p.type}</td>
        <td>${p.method}</td>
        <td><strong style="color: var(--status-available);">+${p.amount} DH</strong></td>
      </tr>
    `).join("");
  },
  ...
  bindGlobalEvents() {
    ...
    if (resetBtn) {
      resetBtn.addEventListener("click", () => {
        if (confirm("Réinitialiser toutes les données d'origine ?")) {
  ...
  runAvailabilitySearch() {
    ...
    if (resultBox) {
      ...
      if (available.length === 0) {
        resultBox.innerHTML = `
          <div style="padding: 12px; margin-top: 12px; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); border-radius: var(--radius-sm); color: #EF4444; font-weight: 700; font-size: 0.85rem;">
            ⚠️ Aucun véhicule disponible pour ces dates. Appelez au ${db.agency.phone1}.
          </div>
        `;
      } else {
        resultBox.innerHTML = `
          <div style="margin-top: 12px; padding: 12px; background: rgba(16, 185, 129, 0.08); border: 1px solid rgba(16, 185, 129, 0.25); border-radius: var(--radius-sm);">
            <div style="font-weight: 800; color: #10B981; margin-bottom: 8px; font-size: 0.85rem;">
              ✓ ${available.length} véhicule(s) disponible(s) :
            </div>
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
              ${available.map(c => `
                <div style="background: var(--bg-surface); padding: 8px 12px; border-radius: var(--radius-sm); border: 1px solid var(--border-subtle); display: flex; align-items: center; gap: 10px; flex: 1; min-width: 200px;">
                  <img src="${c.images[0]}" style="width: 44px; height: 30px; object-fit: cover; border-radius: 4px;" onerror="this.src='assets/images/clio_5_1.jpg'">
                  <div style="flex: 1;">
                    <strong style="color: #FFF; font-size: 0.82rem;">${c.brand} ${c.model}</strong><br>
                    <span style="color: var(--gold-400); font-weight: 700; font-size: 0.78rem;">${c.priceDay} DH/j</span>
                  </div>
  ...
  saveNewCar() {
    ...
    this.showToast(`${brand} ${model} ajouté !`, "success");
    this.refreshAll();
  },

  getStatusBadge(status) {
    const map = {
      confirmee: `<span style="background: rgba(245, 158, 11, 0.15); color: var(--gold-400); border: 1px solid rgba(245, 158, 11, 0.3); padding: 2px 8px; border-radius: 12px; font-size: 0.68rem; font-weight: 800;">CONFIRMÉE</span>`,
      en_cours: `<span style="background: rgba(59, 130, 246, 0.15); color: #60A5FA; border: 1px solid rgba(59, 130, 246, 0.3); padding: 2px 8px; border-radius: 12px; font-size: 0.68rem; font-weight: 800;">EN LOCATION</span>`,
      terminee: `<span style="background: rgba(16, 185, 129, 0.15); color: #10B981; border: 1px solid rgba(16, 185, 129, 0.3); padding: 2px 8px; border-radius: 12px; font-size: 0.68rem; font-weight: 800;">RESTITUÉE</span>`,
      annulee: `<span style="background: rgba(239, 68, 68, 0.15); color: #EF4444; border: 1px solid rgba(239, 68, 68, 0.3); padding: 2px 8px; border-radius: 12px; font-size: 0.68rem; font-weight: 800;">ANNULÉE</span>`
    };
    return map[status] || `<span style="color: var(--text-dim); font-size: 0.7rem;">${status}</span>`;
  },
